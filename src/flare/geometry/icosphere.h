...
namespace flr {
    ...
    public:
        IcoSphere(flr::Mesh & base, flr::f64 radius, flr::u64 subdiv) {
            ...
            for(const auto & vert : base.points()) { center += vert; }

            center /= base.points().size();

            // center = flr::average(base.pts());

            for(const auto & vert : base.points()) {
        ...
        u64 middlepoint(u64 a, u64 b) {
            ...
            if(p != pts.end()) { return p->second; }

            verts.push_back((verts[a] + verts[b]) / 2);

            pts.insert(std::make_pair(key, verts.size() - 1));
